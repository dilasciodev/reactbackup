...
class EditPackMember extends AuthorizedHowlComponent {
  ...
  async returnPackName(){
      return this.returnAsync('HOWL_PACK_NAME');
  }
  async returnPackPhone(){
      return this.returnAsync('HOWL_PACK_PHONE');
  }
  async returnPackEmail(){
      return this.returnAsync('HOWL_PACK_EMAIL');
  }
  async returnPackID(){
      return this.returnAsync('HOWL_PACK_ID');
  }
  ...
  componentWillMount () {
    ...
    let packNAME = this.returnPackName().then(function(result){
      console.log(result);
      AHC.setState({fullName : result});
      console.log(AHC.state);
    });
    let packID = this.returnPackID().then(function(result){
      console.log(result);
      AHC.setState({pack_ID : result});
      //console.log(AHC.state);
    });
  ...
  this.setState({
    ...
    var object = JSON.stringify([{"Email":this.state.packEmail, "FirstName":stringArray[0], "LastName":stringArray[2],"PhoneNumber":this.state.packPhone,"UserPackID":this.state.pack_ID, "PhoneNumberCountryCode": "1"}]);
       console.log(object);
