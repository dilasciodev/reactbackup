...
class CreateCode extends AuthorizedHowlComponent {
  ...
  constructor(props) {
    ...
    async checkText(){
      ...
      let userObject = {
        ...
        "CancellationCode":this.state.mynewcode
      }
      let AHC = this;

      await AsyncStorage.setItem('HOWL_CANCEL_CODE', this.state.mynewcode);

      AHC.authorizedHowlCall("SetCancellationCode", userObject, AHC.cancelFinish);


    }

    cancelFinish(){
      Alert.alert('Cancel Code Added',
        'You have created your cancel code.',[
      {text: 'OK', onPress: () => { Actions.createFakeCode() }},
     ])
    }
